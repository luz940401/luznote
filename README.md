...
1. 開啟「線索收藏」，直接點「＋ 加入線索」。每張卡片的「編輯」也可直接使用；「整理收藏」用於顯示移除按鈕。
2. 填寫標題、類型（信件／道具／圖片／地圖／其他）、取得章節、取得地點和說明。
3. 貼上 Google Drive 圖片分享連結，一行一張；同一本書或同一組道具可放在同一筆線索。權限需設為「知道連結的任何人可查看」。也可先保存純文字。
4. 點「儲存線索」。已連結雲端時會嘗試立即同步；未連線或失敗時保留在本機，恢復連線後按「同步」重試。
5. 玩家重新整理或按同步，即可搜尋、按類型／章節篩選，點卡片閱讀全文、放大圖片或返回來源章節。

多張圖片依輸入順序保存，可在編輯預覽用 ↑／↓ 調整順序或移除單頁；第一張為收藏封面。閱讀器提供頁碼、縮圖選頁及上一頁／下一頁，放大後也能翻頁。底部「上一份線索／下一份線索」則用來切換整份收藏。原有單張圖片會自動當成一頁顯示，編輯儲存後使用 `images` 陣列保存順序，並保留第一張的 `imageUrl` 作為相容封面。

主筆記節點也支援多張 Google Drive 圖片：編輯節點時將連結逐行貼入，並選擇「翻頁顯示」或「一頁多格」。翻頁模式只顯示目前頁面並提供上一頁／下一頁；多格模式在同一筆筆記內以網格顯示全部圖片。舊筆記的 `imageUrl` 會自動視為只有一頁，新增資料保存於 `imageUrls` 與 `imageMode`。
